...
RegisterNetEvent("wonev:givetask", function()
    if not active then
        local mainMenu = {{
            ...
            txt = " Aracın Anahtarı Verilir Ve Diğer Kategorilere Göre Daha Yavaş Araçlar Verilir.",
            params = {
                event = "wonev:StandardLevel"
            }
        }, {
            header = "Orta Level",
            txt = "Aracın Anahtarı Verilir Ve Standart Kategorisine Göre Daha Hızlı Araç Verilir.",
            params = {
                event = "wonev:MediumLevel"
            }
        }, {
            header = "Gelişmiş Level",
            txt = "Aracın Anahtarı Verilmez Ve Diğer Kategorilere Göre Daha Hızlı Araç Verilir.",
            params = {
...
local chopDoorParts = {
    ...
    {part = 1, bone = 'door_pside_f', text = 'Sag Ön Kapı'},
    {part = 2, bone = 'door_dside_r', text = 'Sol Arka Kapı'}, 
    {part = 3,bone = 'door_pside_r',text = 'Sag Arka Kapı'}
}
local OtherParts = {
                    ...
                    if StartOther then
                        for index, part in pairs(OtherParts) do
                            ...
                            if boneindex ~= -1 then
                                sleep = 1
                                local bonecoords = GetWorldPositionOfEntityBone(vehicle, boneindex)
                                local distance2 = GetDistanceBetweenCoords(PlayerCoords.x, PlayerCoords.y, PlayerCoords.z, bonecoords.x, bonecoords.y, bonecoords.z)
                                if distance2 < 2 then
                                    DrawText3D(bonecoords.x, bonecoords.y, bonecoords.z,
                                        "[E] " .. part.text .. "u  Parçala")
                                    if distance2 < 1.5 then
                                        if IsControlJustReleased(0, 38) and not disableaction2 then
...
RegisterNetEvent("wonev:UseJack", function()
    QBCore.Functions.Progressbar("UseJack", "Aracı Havaya Kaldırıyorsun ", 5000, false, true, {
        ...
        object = CreateObject(GetHashKey("prop_carjack"), vehCoords.x, vehCoords.y, vehCoords.z - 0.5, true, true,
            true)
...
local wheelParts = {
    {part = 0, bone = 'wheel_lf'},
    {part = 1, bone = 'wheel_rf'},
    {part = 4, bone = 'wheel_lr'},
    {part = 5, bone = 'wheel_rr'}
}
            ...
            for index, part in pairs(wheelParts) do
                ...
                if boneindex ~= -1 then
                    ...
                    if #(PlayerCoords - bonecoords) < 1.0 then
                        DrawText3D(bonecoords.x, bonecoords.y, bonecoords.z, "[E] Tekerlegi Sök")
                        if IsControlJustPressed(0, 38) then
                            print("Tekerlek sökülüyor...")
                            QBCore.Functions.Progressbar("Remove_Wheels", "Tekerleği Söküyorsun ", 5000, false, true, {
                                ...
                                SetVehicleTyreBurst(vehicle, part.part, true, 1000.0)
                                table.remove(wheelParts, index)
                                ...
                                if #wheelParts == 0 then
                                    ...
                                    vehicle = nil
                                    active = false
                                    VehicleDestroyStarting = false
                                    DeleteEntity(object)
                                    object = nil
                                    if not vehicle then
                                        SelectedStandart = false
                                        SelectedMedium = false
                                        SelectedAdvanced = false
                                    end
                                    -- print(active)
                                    -- print(VehicleDestroyStarting)
                                    
                                    print("Tüm tekerlekler söküldü, araç silindi.")
                                end
                            end, function()
